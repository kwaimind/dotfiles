...
### Step 1
Check if the git log to see if any of the commits in this branch have changed the version in @package.json. If yes, use that version for the following commands. If not, you should run `npm version patch`. Then use this new patch version to create a changelog. DO NOT run `npm install` or `npm ci` OR install any dependencies. 
