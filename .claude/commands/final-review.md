...
PARALLEL_WORKERS=11 bin/rails test 2>&1 > ./tmp/test-results.log  # Full suite
```

### 4b. JavaScript Tests
```bash
npx eslint path/to/changed/files.jsx  # Lint changed JS files
npm test  # Jest suite
```

### 4c. Rails Runner Contract Verification
Use test accounts to verify data contracts with real data. Define these in your CLAUDE.md:
...
### 4d. Chrome Browser Testing
Based on the code changes, identify which pages/features are affected. Then:
1. List out each manual smoke test you would normally ask me to do
2. Use the Chrome automation tools to test each one yourself
3. Login as `sometestemail@address.com` / `thisisnotreallyapassword`
4. Navigate to affected pages on port 3100
5. Verify UI renders correctly, interactions work, data displays properly
